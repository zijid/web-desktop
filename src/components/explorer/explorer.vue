...
import { openFile as openFileAssoc, getOpenWithApps, getAppForFile, getAppForExtension } from '@/system/filetypes'


import FileProperties from "@/components/explorer/FileProperties.vue"
...
const filteredDir = computed(() => {
  ...
  filtered.sort((a, b) => {
    ...
    const aName = (a.system ? a.name : (a.nickname || a.name) || "").toLowerCase()


    const bName = (b.system ? b.name : (b.nickname || b.name) || "").toLowerCase()
...
const sidebarItems = computed(() => {
  ...
  const driveItems = (systemDirectory || [])
    .sort((a, b) => {
      // C盘优先
...
function getFileIcon(file) {
  ...
  if (file.type === "WebFile" && file.extension) {
...
const showFileProps = ref(false)


const propFile = ref(null)


function showProperties(file) { propFile.value = file; showFileProps.value = true }





function isRootPath() { return currentPath.value === '/' || currentPath.value === '' }
async function createNewFolder() {
  if (isRootPath()) { toast?.show?.({ title: '提示', message: '此位置不能新建文件夹', type: 'warning', duration: 2000 }); return }
  await endEdit();
...
async function createNewFile() {
  if (isRootPath()) { toast?.show?.({ title: '提示', message: '此位置不能新建文件', type: 'warning', duration: 2000 }); return }
  await endEdit();
...
function showBlankCtxMenu(e) {
  ...
  function showSidebarCtxMenu(e, child) {
    ...
    const items = [
      ...
      { title: "属性", hander: () => { propFile.value = { name: child.label, path: child.path, type: "WebDir", createTime: Date.now() }; showFileProps.value = true } }
...
function buildMenuItems(file) {
  ...
  const singleFile = selectedFiles.value.length > 0 ? selectedFiles.value[0] : null
    ...
    items.push({ title: "属性", hander: () => { const pathParts = currentPath.value.split("/").filter(Boolean); propFile.value = { name: pathParts[pathParts.length-1] || "此电脑", path: currentPath.value, type: "WebDir", createTime: Date.now() }; showFileProps.value = true } })
...
function onDragOver(e) {
  dragOverCount.value++
  if (contentAreaRef.value) contentAreaRef.value.classList.add('drag-over')
}
...
function onDragLeave(e) {
  ...
  if (dragOverCount.value <= 0) {
    ...
    if (contentAreaRef.value) contentAreaRef.value.classList.remove('drag-over')
  }
}
...
function onDrop(e) {
  ...
  const toastMsg = toast.show({ title: 'Importing...', message: 'Processing', type: 'info', duration: 0 })
  let saved = 0
  let errors = 0
  ...
  function checkDone() {
    ...
    if (done >= total) {
      toastMsg.close && toastMsg.close()
      if (saved > 0) {
        toast.show({ title: 'Import complete', message: saved + ' files imported' + (errors > 0 ? ', ' + errors + ' failed' : ''), type: 'success', duration: 3000 })
        refresh()
      } else {
        toast.show({ title: 'Import failed', message: 'Could not import any files', type: 'error', duration: 3000 })
      }
    }
  }
}
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = async function(e) {
      try {
        ...
        const destPath = currentPath.value + '/' + name
        ...
        if (exists) {
          ...
          let counter = 2
          while (await readFile(currentPath.value + '/' + baseName + '(' + counter + ')' + suffix)) {
            counter++
          }
          finalName = baseName + '(' + counter + ')' + suffix
        }
        
        const wf = new WebFile(currentPath.value, finalName)
        
        // Save all files as data URL (base64)
        const isImage = ['jpg','jpeg','png','gif','bmp','webp','svg','ico','tiff','tif','avif','heif','heic'].includes(ext)
        if (isImage) {
          wf.write(dataUrl)
        } else {
          wf.write(dataUrl)
        ...
        await initList(currentPath.value)
        
        resolve()
      } catch (err) {
        console.error('[Explorer] Import failed:', err)
        reject(err)
      }
    }
...
<Win :path="path" :pid="pid" @keydown.stop="onKeydown">
  ...
  <div class="explorer-win10">
    ...
    <div class="explorer-body">
      ...
      <div class="content-area" ref="contentAreaRef" @click.self="clearSelection" @contextmenu.prevent.stop="showBlankCtxMenu" @dragover.prevent="onDragOver" @dragleave.prevent="onDragLeave" @drop.prevent="onDrop">
  ...
  <Teleport to="body">
    ...
    <FileProperties :visible='showFileProps' :file='propFile' @close='showFileProps=false' />
...
.ribbon-btn.ribbon-disabled { opacity: 0.4; pointer-events: none; }
