...
const db = {
  ...
  async createTable() {},
  async add(t, d, k) { this.data.set(k, d); return d },
  async find(t, k) { return this.data.get(k) || null },
  async findIndexAll(t, idx, val) { return Array.from(this.data.values()).filter(d => d._pwd === val) },
  async delete(t, k) { this.data.delete(k) }
}
