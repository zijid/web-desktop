...
const db = (() => {
  const STORAGE_KEY = "web-desktop-files"
  const data = new Map()
  ...
  function restore() {
    ...
    try {
      ...
      if (saved) {
        ...
        if (Array.isArray(entries)) entries.forEach(([k, v]) => data.set(k, v))
      }
    } catch(e) { /* ignore */ }
  }
  ...
  return {
    ...
    async findIndexAll(t, idx, val) {
      restore()
      return Array.from(data.values()).filter(d => d._pwd === val)
    },
    async delete(t, k) {
      data.delete(k)
      persist()
...
export function dbToFile(params,newFile=false){
	...
	if(params.type==="WebFile"){
		...
		if(params.icon)dir.setIcon(params.icon)
		return dir
	}
}
...
async function testFile(){
	...
	// 始终重新创建，清理 localStorage 中可能存在的乱码缓存
	// 先删除旧的初始化数据
	for(const old of initDir){
		const f = await readFile(old.pwd + "/" + old.name)
		if(f) await f.delete()
	}
	for(const old of initFile){
		const f = await readFile(old.pwd + "/" + old.name)
		if(f) await f.delete()
	}
	// 重新创建正确的目录和文件
