...
function getFileIcon(file) {
  ...
  if (file.type === "WebDir") {
    if (file.system) {
      return '<svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 33H4V7H44V33H36H12Z" fill="none" stroke="#0078d4" stroke-width="4" stroke-linejoin="round"/><path d="M16 22V26" stroke="#0078d4" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/><path d="M24 18V26" stroke="#0078d4" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/><path d="M32 14V26" stroke="#0078d4" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/><path d="M12 41H36" stroke="#0078d4" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/></svg>'
    }
...
async function createNewFolder() {
  ...
  try {
    await ndir.save()
    system.initList(currentPath.value).then(() => {
      const list = fileList[currentPath.value]
      if (list) { const created = list.find(f => f.uid === ndir.uid); if (created) startEdit(created) }
    })
...
async function createNewFile() {
  ...
  try {
    ...
    system.initList(currentPath.value).then(() => {
      const list = fileList[currentPath.value]
      if (list) { const created = list.find(f => f.uid === nfile.uid); if (created) startEdit(created) }
    })
    ...
    <div class="explorer-body">
      <div class="sidebar" v-show="showSidebar">
        <div v-for="group in sidebarItems" :key="group.key" class="sidebar-group">
          <div class="sidebar-group-header" @click="sidebarExpanded[group.key] = !sidebarExpanded[group.key]">
      ...
      <div class="content-area" @click.self="clearSelection" @contextmenu.prevent="showBlankCtxMenu">
        ...
        <div v-else-if="viewMode === 'icons'" class="icons-view">
          <div v-for="file in filteredDir" :key="file.uid"
            ...
            <div class="file-name" v-if="!(editFile && editFile.uid === file.uid)" :title="file.nickname || file.name">{{ file.nickname || file.name }}</div>
            <div class="file-name edit" v-else contenteditable v-focus
        ...
        <div v-else-if="viewMode === 'list'" class="list-view">
          <div v-for="file in filteredDir" :key="file.uid"
            ...
            <span class="file-name-text" v-if="!(editFile && editFile.uid === file.uid)">{{ file.nickname || file.name }}</span>
            <span class="file-name-text edit" v-else contenteditable v-focus
              @input="onEditNameInput" @click.stop @dblclick.stop
        ...
        <div v-else-if="viewMode === 'details'" class="details-view">
          ...
          <div v-for="file in filteredDir" :key="file.uid"
            ...
            @click.stop="toggleSelect($event, file)" @dblclick="openFile(file)"
            @contextmenu.prevent.stop="showCtxMenu($event, file)">
            <span class="col-name"><span class="file-icon-tiny" v-html="getFileIcon(file)"></span><span v-if="!(editFile && editFile.uid === file.uid)" class="name-text">{{ file.nickname || file.name }}</span><span v-else contenteditable class="name-text edit" v-focus @input="onEditNameInput" @click.stop @dblclick.stop @keydown="onEditNameKeydown" @blur="endEdit">{{ editFileName }}</span></span>
...
.sidebar-item-icon { margin-right: 6px; font-size: 14px; }
.sidebar-item-label { }
/* Content area */
.content-area {
...
.file-item.icons {
  display: flex; flex-direction: column; align-items: center;
  width: 80px; padding: 6px 4px; cursor: default;
  border: 1px solid transparent; border-radius: 3px;
}
.file-item.icons:hover { background: #e8f0fe; }
.file-item.icons.selected { background: #c0e0ff; border-color: #0078d4; }
.file-item.icons .file-icon { width: 48px; height: 48px; margin-bottom: 4px; }
.file-item.icons .file-icon svg { width: 48px; height: 48px; }
