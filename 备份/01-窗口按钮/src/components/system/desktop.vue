	...
	if(i.type==="WebDir"||i.type==="WebFile"){
		...
		if(i.type==="WebDir"||i.type==="WebFile"){
			menuDatas.value.push({
				title:"查看源代码",
				hander:()=>{
					launchApp('editor', { title: '查看源代码: ' + i.name, args: i.path })
				}
			})
			menuDatas.value.push({
				title:"属性",
...
const menuData=[
	{
		title:"查看源代码",
		icon:`<?xml version="1.0" encoding="UTF-8"?><svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M34 6H14C9 6 6 9 6 14V34C6 39 9 42 14 42H34C39 42 42 39 42 34V14C42 9 39 6 34 6Z" fill="none" stroke="#333" stroke-width="4"/><path d="M27 16L33 24L27 32" stroke="#333" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/><path d="M21 16L15 24L21 32" stroke="#333" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/></svg>`,
		hander:()=>{
			launchApp('editor', { title: '查看源代码', args: window.location.href })
		}
	},
	{
		title:"属性",
	...
	<div @focus="f" ref="desktop" tabindex="1" class="desktop" :style="{backgroundImage:`url(${bg})`}" @click.stop="selectAppListEmpty" @click="editNameBlue" @contextmenu.prevent="showMenu($event,{alias:'desktop',path:config.desktop.path})">
		...
		<div id="windows">
			...
			<template v-for="(progress,index) in progressList" :key="progress.pid">
			  ...
			  <AppFrame v-else v-bind="progress" ref="app" @close="sysCloseWindow" @focus="(pid) => showWindow(pid, 'window')" />
			</template>
		</div>
