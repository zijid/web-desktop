...
import { scanAndRegisterApps } from "@/system/apps/scanner";

export const bus=new Bus()
export let systemDirectory
export async function initApp(){
	const res = await loadSystemFile(getConfig())
	systemDirectory=res
}

/**
 * 初始化新应用系统
 * 自动扫描注册所有应用，建议在应用启动时调用
 */
let _appSysInitialized = false
export async function initAppSystem() {
  if (_appSysInitialized) return
  await scanAndRegisterApps()
  _appSysInitialized = true
  console.log('[AppSystem] 应用系统初始化完成')
}
...
export const selectAppList=ref([])//选中的应用
